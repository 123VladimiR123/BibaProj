...
#define SETUP_TESTCASES() \
    ...
    SECTION("retrieve start position and end position")
    {
        SECTION("for object")
        {
            ...
            auto expected = json({{"nested", {{"a", 1}, {"b", "test1"}}}, {"anotherValue", "test"}});
        ...
        {
            const std::string nested_type_json_str =  R"([{"a": 1, "b": "test"}, {"c": 2, "d": "test"}])";
            const std::string root_type_json_str =  R"({   "nested": )" + nested_type_json_str + R"(, "anotherValue": "test" })";
            auto expected = json({{"nested", {{{"a", 1}, {"b", "test"}}, {{"c", 2}, {"d", "test"}}}}, {"anotherValue", "test"}});
            auto filteredExpected = expected;
